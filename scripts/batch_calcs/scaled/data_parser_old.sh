...
if [[ ${#AVAILABLE_FUNCTIONALS[@]} -eq 0 || ${#AVAILABLE_CALCS[@]} -eq 0 ]]; then
    echo "${RED}No valid POSCAR_scaled_* hierarchy found — nothing to do.${RESET}"
    exit 1
fi
...
for CALC in "${SELECTED_CALCS[@]}"; do
    for FUNC in "${SELECTED_FUNCS[@]}"; do
        ...
        echo -e "Directory\tGap_eV\tFermi_E_eV" > "$out_dir_abs/band_gaps.dat"
        ...
        for poscar in POSCAR_scaled_*; do
            ...
            # PATCHED LINES (safe awk parsing)
            ...
            electrons=$(awk '{if (NF > 1) print $(NF-1); else print "NA"}' <<<"$fermi_line")
            band_energy=$(awk '{if (NF > 2) print $3; else print "NA"}' <<<"$band_line")
            bloehl_corr=$(awk '{if (NF > 6) print $7; else print "NA"}' <<<"$band_line")
            ...
            (
                ...
                if [[ -f BAND_GAP ]]; then
                    ...
                    echo -e "${tag}\t${gap_val:-NA}\t${fermi_val:-NA}" >> "$out_dir_abs/band_gaps.dat"
                fi
