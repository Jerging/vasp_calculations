...
if [ "$#" -ne 6 ]; then
    echo "Usage: $0 <POSCARDIR> <SYSTEM> <CALC> <KPR> <KSCHEME> <FUNCTIONAL>"
    exit 1
fi
...
mkdir -p "${SYS}_calculations" || { echo "Failed to create \"calculations\" directory."; exit 1; }

if [ ! -d "POSCARs/$POSCARDIR" ]; then
    echo "Error: POSCARs/$POSCARDIR directory not found."
    exit 1
fi
cp "POSCARs/$POSCARDIR/POSCAR" "${SYS}_calculations/${POSCARDIR}_POSCAR"
...
for poscar_file in "POSCARs/$POSCARDIR"/POSCAR_scaled_*; do
    ...
    scaled_dir="${SYS}_calculations/${fname}"
